...
class cLoot
{
    ...
    // Spawn random loot, also spawn HP bubbles if player is low HP
    void SpawnRandom(Entity @player, Vec3 source)
    {
        ...
        // drop extra HP if player is low hp
        if (player.health <= 50){
            // drop one 25 hp, and two 5 hp
            DropItem(@player, source, HEALTH_MEDIUM).spawnFlags |= ITEM_IGNORE_MAX;
            DropItem(@player, source, HEALTH_SMALL).spawnFlags |= ITEM_IGNORE_MAX;
            DropItem(@player, source, HEALTH_SMALL).spawnFlags |= ITEM_IGNORE_MAX;
        }
